...
def epsSR(x, a, b, v, l):  # Flow parameter, slow-roll approximation
    ...
    #return 8*MP/v**2*(3 - 3*a*x +3*(1-b)*x**2+a*b*x**3)**2/x**2/(6 - 4*a*x + 3*x**2)**2/(1+b*x**2)**2
...
def etaSR(x, a, b, v, l):  # Second Hubble flow parameter, slow-roll
    ...
    # return 4*epsSR(x, a, b, v, l) - 2*MP**2*(pot_dd(x, a, b, v, l)/v**2)/pot(x, a, b, v, l)

''' INFLATION '''
# def systemsolvSR(S0, N, a, b, v, l):
#     solution = odeint(systemSR, S0, N, args=(a, b, v, l))
#     return solution
...
def mukhanovsolve(S, k, d, a, b, v, l):
    ...
    #def solveN(k, Ni, cond):
    #    fz = lambda N: cond*Hc(N) - k
    #    Ns = root(fz, Ni).x
    #    return Ns
...
def power(u, v, z, k):
    return k**3*np.abs((u[-1] + 1j*v[-1])/z[-1])**2

...
def plot_epsilon(S0, x_dN, d, a, b, v, l, font=13):
    ...
    ax[0].plot((NfSR), epsintSR(NfSR), '-', color='tomato', label='Slow-Roll') # [ind2]
    #ax[0].plot(NfoldsSR_zoom, epsSR_zoom, '-', color='tomato')  # [ind3]
    #ax[0].plot(Nfolds[25:], epsilonexact[25:], color='cornflowerblue', label='Exact')
    ax[0].plot((Nf), epsint(Nf), color='cornflowerblue', label='Exact') # [ind]
    ...
    ax[1].set_yscale('log') 
    ax[1].plot(NfSR, etaintSR(NfSR), '-', color='yellowgreen', label='Slow-Roll')  # [ind2]
    #ax[1].plot(Nfolds[25:], etaexact[25:], color='teal', label='Exact')
    ax[1].plot(Nf, etaint(Nf), color='teal', label='Exact') # [ind]
...
def plot_efolds(S0, x_dN, d, a, b, v, l, font=13):
    ...
    #ax[0].plot(-efoldsSRv(x_vals, x_vals[0], a, b, v, l)*v, x_vals, '--', color='green', label='Slow-Roll')
...
x_0 =  9.27 #11.72  # Initial inflaton field value
x_dot_0 = -1.413e-1  # Initial velocity of inflaton field
# H_0 = ((.5*x_dot_0**2*v0**2 + pot(x_0, a0, b0, v0, l0))/3)**.5/MP  # Constraint
